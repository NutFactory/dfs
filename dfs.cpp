...
void dfs(unsigned v, Graph& g, unsigned& count, unsigned& comp)
{
	count++;
	g.V[v].visited = count;
	g.V[v].component = comp;
...
void mainDFS(const Edges& E)
{
  ...
  {
    cerr << i<<" visited "<< G.V[i].visited << " Component : " << G.V[i].component << endl;
   }
